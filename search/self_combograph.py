...
def NeighborFinder(Graph, ComboNode):  # Output edges connected to this ComboNode
    neighbors = np.empty(
        shape=(0, len(ComboNode)), dtype=int
...
def ComboSubgraph_Constructor(
    ...
    for ComboNode in ComboNode_list:  
        ...
        if ComboSubgraph.number_of_nodes() >= Q and fast_computation:
            break
    ...
    if ComboSubgraph.number_of_nodes() <= Q and len(new_neighbors) > 0 and l < l_max:  
        ...
        if len(ComboSubgraph) > Q:
            nodes_to_remove = random.sample(
                new_neighbors, k=ComboSubgraph.number_of_nodes() - Q
            )
        ...
        print(
            f"Current size of the ComboSubgraph: {len(ComboSubgraph)}. Time for computation: {(time()-start_time):.1f}s"
        )
